    ...
    */
    this.dinosaurs = (function(){
        ...
        for(const dino of dinoData["Dinos"]){
            // append 8 dinosaur objects into array
...
* @constructor
* @param {string} name - Name of human user
...
/**
* @description Get user properties in array
* @returns {array} array of user properties
*/
...
/**
* @description Get properties of dinosaur in array
* @returns {array} array of properties of dinosaur
*/
...
/**
* @description Get all animals including human in array
* @returns {array} an array of all animal objects
*/
Canvas.prototype.getAnimals = function(){
    ...
    // and add human object to this array
...
/**
* @description Generate a tile for an animal: includes div, which contains
* header, paragraph (only for dinosaur), and image tag
...
Canvas.prototype.createTile = function(animal){
    ...
    // add class grid-item to div tag
    ...
    // and append div tag to main tag
...
* @param {object} animal - A dinosaur or human object
* @param {string} content - Fact to display (no fact for human tile)
*/
...
* 4 in the center of array (center reserved for human)
...
Canvas.prototype.createRandomIntArray = function(){
    ...
    while(arr.length < scale - 1){   // until array is full
        ...
        // center index reserved for human and only distinct integers in array
        if(randomInt !== 4 && arr.indexOf(randomInt) === -1){
            arr.push(randomInt);
        }
    }
    // place integer 4 in center of array (human is in fixed position)
    arr.splice(4, 0, 4);
...
* @param {number} - cases from 0 to 4 has units, otherwise unitless
...
Canvas.prototype.displayAnimalFact = function(animal){
    ...
    if(animal.species === 'Human'){
        // if a human, then there is no fact just user name
        this.displayContent(animal, '');
    } else if(animal.species === 'Pigeon'){
        // if a pigeon, then display the one given fact with name
        this.displayContent(animal, animal.fact);
    } else{
        // if a dinosaur (and not a pigeon), then display any fact with name
...
button.addEventListener('click', function(){
    ...
    if(canvas.isDataValid()){
        ...
        alert("Enter valid Name, Height, and Weight");
    }
